    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-white flex items-center gap-2">
            <Headset className="w-6 h-6 text-red-400" />
            Support Inbox
          </h1>
          <p className="text-sm text-slate-400 mt-1">Manage player issues and internal requests</p>
        </div>
        <Button 
          onClick={() => router.push('/admin/support/internal/new')}
          className="bg-amber-600 hover:bg-amber-500 text-white shadow-lg rounded-xl"
        >
      ...
      <div className="flex gap-2 bg-slate-900/50 p-1.5 rounded-xl border border-white/5 w-max">
        ...
        <button
          ...
          className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-colors ${filter === 'mine' ? 'bg-white/10 text-white' : 'text-slate-400 hover:text-slate-300'}`}
        >
          My Assigned
        </button>
        <button
          onClick={() => setFilter('unassigned')}
          className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-colors ${filter === 'unassigned' ? 'bg-white/10 text-white' : 'text-slate-400 hover:text-slate-300'}`}
        >
          Unassigned
        </button>
      </div>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {tickets.map(ticket => (
            <div 
              ...
              className="glass-panel p-5 rounded-2xl border border-white/10 hover:border-red-500/30 transition-all cursor-pointer relative overflow-hidden group"
            >
              ...
              <h3 className="text-base font-bold text-white group-hover:text-red-400 transition-colors mb-1">
                {ticket.subject}
              </h3>
